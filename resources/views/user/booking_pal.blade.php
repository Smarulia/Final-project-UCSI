...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <form action="{{route("user.post.booking.pal")}}" method="POST">
            @csrf
            ...
            <div class="form-group">
                <label for="jurusan_pal-type">Course Name </label>
            ...
            <div class="form-group">
                <label for="pal-type">PAL</label>
                <select class="form-control" id="pal" name="pal_id" required>
                    <option value="">Select Your PAL</option>
                    @foreach ($majors as $item)
                    <option value="{{$item->id}}">{{$item->name}}</option>
                    @endforeach
                </select>
